...
module.exports = function checkUniqueness(options, identifyUser, ownId, meta) {
  ...
  const keys = Object.keys(identifyUser).filter(key => identifyUser[key] !== undefined && identifyUser[key] !== null);

  return Promise.all(keys.map(prop => users.find({ query: { [prop]: identifyUser[prop].trim() } }).then(data => {
