...
module.exports = function sendResetPwd(options, identifyUser, notifierOptions) {
  ...
  return Promise.resolve().then(() => {
    ...
    return Promise.all([users.find({ query: identifyUser }).then(data => getUserData(data, checkProps)), getLongToken(options.longTokenLen), getShortToken(options.shortTokenLen, options.shortTokenDigits)]);
  }).then(([user, longToken, shortToken]) => Object.assign(user, {
    resetExpires: Date.now() + options.resetDelay,
    resetToken: concatIDAndHash(user[usersIdName], longToken),
    resetShortToken: shortToken
  })).then(user => notifier(options.notifier, 'sendResetPwd', user, notifierOptions).then(() => user)).then(user => Promise.all([user, hashPassword(options.app, user.resetToken), hashPassword(options.app, user.resetShortToken)])).then(([user, longToken, shortToken]) => patchUser(user, {
