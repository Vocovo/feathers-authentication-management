...
function resetPassword(options, query, tokens, password) {
  ...
  return Promise.all([userPromise, hashPassword(options.app, password)]).then(([user, hashPassword]) => {
    let promises = [];
    ...
    return patchUser(user, {
      ...
      resetShortToken: null,
      resetExpires: null
    }).then(user1 => notifier(options.notifier, 'resetPwd', user1)).then(user1 => sanitizeUserForClient(user1));
  });
