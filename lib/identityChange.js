...
module.exports = function identityChange(options, identifyUser, password, changesIdentifyUser) {
  ...
  return Promise.resolve().then(() => {
    ...
    return users.find({ query: identifyUser }).then(data => Array.isArray(data) ? data[0] : data.data[0]);
  }).then(user1 => Promise.all([user1, getLongToken(options.longTokenLen), getShortToken(options.shortTokenLen, options.shortTokenDigits), comparePasswords(password, user1.password, () => new errors.BadRequest('Password is incorrect.', { errors: { password: 'Password is incorrect.', $className: 'badParams' } }))])).then(([user1, longToken, shortToken]) => {
    const patchToUser = {
  ...
  }).then(user1 => notifier(options.notifier, 'identityChange', user1, null)).then(user1 => sanitizeUserForClient(user1));
